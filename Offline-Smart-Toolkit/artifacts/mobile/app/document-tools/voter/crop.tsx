...
import { cropToVoterSize } from '@/lib/features/documents/voter/voterService';

const COLOR = '#8B5CF6';

export default function VoterCropScreen() {
  ...
  return (
    <ToolScreenLayout title="Voter ID Crop" subtitle="Crop to 85.6×54mm standard size" iconName="crop" color={COLOR} onReset={reset}>
        ...
        <>
          ...
          {file && (
            ...
            >
              ...
              <Text style={[styles.btnText, { color: '#fff', fontFamily: 'Inter_600SemiBold' }]}>
                {processing ? 'Processing…' : 'Crop to Voter ID Size'}
              </Text>
            </TouchableOpacity>
          )}
          ...
          <View style={[styles.infoBox, { backgroundColor: COLOR + '12', borderColor: COLOR + '30', borderRadius: colors.radius }]}>
            ...
            <Text style={[styles.infoText, { color: colors.foreground, fontFamily: 'Inter_400Regular' }]}>
              Standard Voter ID (EPIC) dimensions: 85.6 × 54mm. The image will be center-cropped and resized to 1012 × 638px at 300 DPI.
            </Text>
          </View>
        </>
      ) : (
        <>
          <View style={[styles.resultBox, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius }]}>
            ...
            <View style={styles.resultRow}>
              ...
              <Text style={[styles.resultKey, { color: colors.mutedForeground, fontFamily: 'Inter_400Regular' }]}>Card dimensions</Text>
            ...
            <View style={styles.resultRow}>
              ...
              <Text style={[styles.resultVal, { color: colors.foreground, fontFamily: 'Inter_600SemiBold' }]}>JPEG · 95% quality</Text>
            </View>
          </View>
          <DocResultActions uri={result.uri} fileName="voter-cropped.jpg" color={COLOR} onReset={reset} mimeType="image/jpeg" />
        </>
      )}
    </ToolScreenLayout>
  );
