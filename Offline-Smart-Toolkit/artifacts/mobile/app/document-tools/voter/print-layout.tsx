...
export default function VoterPrintLayoutScreen() {
  ...
  const process = async () => {
    ...
    try {
      const out = await buildFrontBackSheet(front.uri, back?.uri ?? null, 85.6, 54, layout.copies, layout.paperSize);
  ...
  return (
    <ToolScreenLayout title="Print Layout" subtitle="Create Voter ID print sheet" iconName="printer" color={COLOR} onReset={reset}>
      {error && <StatusBanner type="error" message={error} />}

      {!result ? (
        <>
          <Text style={[styles.label, { color: colors.foreground, fontFamily: 'Inter_600SemiBold' }]}>Front Side</Text>
          <DocUploadWidget file={front} onPicked={setFront} onError={setError} color={COLOR} accept="image" label="Upload Front Side" />

          <Text style={[styles.label, { color: colors.foreground, fontFamily: 'Inter_600SemiBold' }]}>Back Side (Optional)</Text>
          <DocUploadWidget file={back} onPicked={setBack} onError={setError} color={COLOR} accept="image" label="Upload Back Side" />
          ...
          <View style={[styles.infoBox, { backgroundColor: COLOR + '12', borderColor: COLOR + '30', borderRadius: colors.radius }]}>
            ...
            <Text style={[styles.infoText, { color: colors.foreground, fontFamily: 'Inter_400Regular' }]}>
              Front and back are placed side-by-side on each sheet. If no back is uploaded, only the front will be repeated. Cards are sized to 85.6×54mm.
            </Text>
          </View>
        </>
      ) : (
        <>
          ...
          <DocResultActions uri={result.pdfUri} fileName="voter-print-layout.pdf" color={COLOR} onReset={reset} mimeType="application/pdf" />
        </>
      )}
    </ToolScreenLayout>
  );
