...
import { VOTER_TOOLS } from '@/lib/features/documents/tools';
import type { DocToolMeta } from '@/lib/features/documents/types';

const COLOR = '#8B5CF6';

export default function VoterToolsHome() {
  ...
  const filtered = VOTER_TOOLS.filter(t => t.name.toLowerCase().includes(query.toLowerCase()));
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: topPadding + 10, borderBottomColor: colors.border, backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.title, { color: colors.foreground, fontFamily: 'Inter_700Bold' }]}>Voter ID Tools</Text>
        <View style={[styles.badge, { backgroundColor: COLOR + '18' }]}>
          <Text style={[styles.badgeText, { color: COLOR, fontFamily: 'Inter_600SemiBold' }]}>{VOTER_TOOLS.length} Tools</Text>
        </View>
      </View>
      <View style={[styles.searchBar, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius }]}>
        <Feather name="search" size={16} color={colors.mutedForeground} />
        <TextInput style={[styles.searchInput, { color: colors.foreground, fontFamily: 'Inter_400Regular' }]} placeholder="Search voter ID tools..." placeholderTextColor={colors.mutedForeground} value={query} onChangeText={setQuery} />
      </View>
