...
export default function DlFrontCropScreen() {
  ...
  const process = async () => {
    ...
    try {
      ...
      const out = await cropToDlSize(file.uri, w, h, 'front');
      setResult(out);
    } catch (e: any) {
  ...
  return (
    <ToolScreenLayout title="DL Front Crop" subtitle="Crop front of Driving License" iconName="crop" color={COLOR} onReset={reset}>
      {error && <StatusBanner type="error" message={error} />}

      {!result ? (
        <>
          <DocUploadWidget file={file} onPicked={setFile} onError={setError} color={COLOR} accept="image" label="Upload DL Front Side" />
          ...
          {file && (
            ...
            >
              ...
              <Text style={[styles.btnText, { color: '#fff', fontFamily: 'Inter_600SemiBold' }]}>
                {processing ? 'Processing…' : 'Crop Front to Standard Size'}
              </Text>
            </TouchableOpacity>
          )}
          ...
          <View style={[styles.infoBox, { backgroundColor: COLOR + '12', borderColor: COLOR + '30', borderRadius: colors.radius }]}>
            ...
            <Text style={[styles.infoText, { color: colors.foreground, fontFamily: 'Inter_400Regular' }]}>
              Standard Indian Driving License front dimensions: 85.6 × 53.98mm (CR80). Output is 1012 × 638px at 300 DPI.
            </Text>
          </View>
        </>
      ) : (
        <>
          <View style={[styles.resultBox, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius }]}>
            ...
            <View style={styles.resultRow}>
              ...
              <Text style={[styles.resultVal, { color: colors.foreground, fontFamily: 'Inter_600SemiBold' }]}>Front</Text>
            </View>
            <View style={styles.resultRow}>
          ...
          <DocResultActions uri={result.uri} fileName="dl-front-cropped.jpg" color={COLOR} onReset={reset} mimeType="image/jpeg" />
        </>
      )}
    </ToolScreenLayout>
  );
