...
export default function DlCopiesScreen() {
  ...
  const process = async () => {
    ...
    try {
      const out = await buildIdCardSheet(file.uri, 85.6, 53.98, copies, paperSize);
  ...
  return (
    <ToolScreenLayout title="Multiple Copies" subtitle="Print DL copies on A4" iconName="content-copy" color={COLOR} onReset={reset}>
      {error && <StatusBanner type="error" message={error} />}

      {!result ? (
        <>
          <DocUploadWidget file={file} onPicked={setFile} onError={setError} color={COLOR} accept="image" label="Upload Driving License Image" />
          ...
          <View style={[styles.infoBox, { backgroundColor: COLOR + '12', borderColor: COLOR + '30', borderRadius: colors.radius }]}>
            ...
            <Text style={[styles.infoText, { color: colors.foreground, fontFamily: 'Inter_400Regular' }]}>
              DL cards are tiled at 85.6×53.98mm with cut guides. Use front-crop or back-crop first for best results.
            </Text>
          </View>
        </>
      ) : (
        <>
          ...
          <DocResultActions uri={result.pdfUri} fileName={`dl-${result.copies}-copies.pdf`} color={COLOR} onReset={reset} mimeType="application/pdf" />
        </>
      )}
    </ToolScreenLayout>
  );
