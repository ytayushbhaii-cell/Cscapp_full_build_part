...
import { cropToPanSize } from '@/lib/features/documents/pan/panService';

const COLOR = '#06B6D4';

export default function PanCropScreen() {
  ...
  return (
    <ToolScreenLayout title="PAN Crop" subtitle="Crop to 85.6×54mm PAN size" iconName="crop" color={COLOR} onReset={reset}>
      ...
      {!result ? (
        <>
          ...
          {file && (
            ...
            >
              ...
              <Text style={[styles.btnText, { color: '#fff', fontFamily: 'Inter_600SemiBold' }]}>
                {processing ? 'Processing…' : 'Crop to PAN Size'}
              </Text>
            </TouchableOpacity>
          )}
          ...
          <View style={[styles.infoBox, { backgroundColor: COLOR + '12', borderColor: COLOR + '30', borderRadius: colors.radius }]}>
            ...
            <Text style={[styles.infoText, { color: colors.foreground, fontFamily: 'Inter_400Regular' }]}>
              Standard PAN card dimensions: 85.6 × 53.98mm (CR80). The image will be center-cropped and resized to 1012 × 638px at 300 DPI.
            </Text>
          </View>
        </>
      ) : (
        <>
          ...
          <DocResultActions uri={result.uri} fileName="pan-cropped.jpg" color={COLOR} onReset={reset} mimeType="image/jpeg" />
        </>
      )}
    </ToolScreenLayout>
  );
