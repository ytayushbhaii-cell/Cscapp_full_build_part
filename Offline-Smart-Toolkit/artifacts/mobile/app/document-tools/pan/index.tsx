...
export default function PanToolsHome() {
  ...
  const filtered = PAN_TOOLS.filter(item => dn(item).toLowerCase().includes(query.toLowerCase()));
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: topPadding + 10, borderBottomColor: colors.border, backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.title, { color: colors.foreground, fontFamily: 'Inter_700Bold' }]}>{t('category.panTools')}</Text>
