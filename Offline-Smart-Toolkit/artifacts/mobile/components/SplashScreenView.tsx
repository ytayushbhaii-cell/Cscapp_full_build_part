/**
 * Custom Splash / Loading screen matching the design reference:
 ...
 *
 * All dimensions are computed inside the component via useWindowDimensions()
...
export default function SplashScreenView({ progress: externalProgress }: Props) {
  ...
  // All layout values derived from live window dimensions
    ...
    <View style={styles.root}>
      ...
      <View style={[styles.loadingArea, { top: H * 0.87 }]}>
        ...
        <View style={{
          ...
          {/* Gradient fill revealed by animated width */}
          <Animated.View style={{
...
function IconIllustration({ size, border }: { size: number; border: number }) {
  ...
  // Unit coords → pixel coords inside the inner box
